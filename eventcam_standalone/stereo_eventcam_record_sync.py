...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="auto",
        help=(
            "When the saved interval begins. 'auto' starts it as soon as both "
            "streams are running, after --start-delay-sec. 'enter' keeps both "
            "cameras streaming and starts the interval when the operator presses "
            "Enter in the terminal; with --duration-sec 0 a second Enter ends it. "
            "Either way the interval is still defined and aligned in the Master "
            "camera clock, so left and right keep the identical saved window."
        ),
    )
...
# Used as the upper bound while a manually stopped interval is still open.
OPEN_INTERVAL_END_TS_US = int(np.iinfo(np.int64).max)


def compute_common_interval_start(current_ts_us: int, lead_us: int, delta_t_us: int) -> int:
    """Master-clock timestamp both cameras will use as the saved interval start.

    The lead never drops below ten acquisition slices, so the Slave always has
    the shared value before the interval opens, and the result is aligned to the
    slice quantum so both sides cut on the same boundary.
    """
    quantum = max(1, int(delta_t_us))
    return align_up(int(current_ts_us) + max(int(lead_us), 10 * quantum), quantum)


def compute_common_interval_end(current_ts_us: int, start_ts_us: int, delta_t_us: int) -> int:
    """Master-clock timestamp that closes a manually stopped interval.

    Uses the same lead and alignment rules as the start, and never returns an
    end at or before the start, so a very fast second Enter still saves events.
    """
    quantum = max(1, int(delta_t_us))
    candidate = align_up(int(current_ts_us) + 10 * quantum, quantum)
    return max(candidate, int(start_ts_us) + quantum)
...
def prompt_operator(prompt: str) -> bool:
    """Block until the operator answers. False means abort instead of continue."""
    try:
        answer = input(prompt)
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    return answer.strip().lower() not in {"q", "quit", "abort"}
...
def record_worker(
    *,
    ...
    start_trigger_event: Any = None,
    stop_trigger_event: Any = None,
    capture_end_ready_event: Any = None,
    shared_capture_end_ts_us: Any = None,
) -> None:
    ...
    try:
        ...
        operator_triggered = start_trigger_event is not None
        manual_stop = float(duration_sec) <= 0
        duration_us = max(1, int(round(float(duration_sec) * 1_000_000)))
        # The operator has already watched the stream while waiting, so the
        # post-trigger lead only needs to cover the Slave handoff.
        start_delay_us = 0 if operator_triggered else max(0, int(round(float(start_delay_sec) * 1_000_000)))
        ...
        for events in iterator:
            ...
            if hardware_synchronized:
                if operator_triggered and not start_trigger_event.is_set():
                    # Both cameras keep streaming while the operator decides, and
                    # nothing is armed yet. Hold the synchronization timeout so it
                    # cannot expire during a wait that has no deadline.
                    stream_started_perf_ns = now_ns
                    continue

                if sync_mode_applied == "master" and capture_ts_ready_event is not None:
                    if not capture_ts_ready_event.is_set():
                        common_start = compute_common_interval_start(
                            int(iterator.get_current_time()), start_delay_us, int(delta_t_us)
                        )
                ...
                if manual_stop and capture_end_ts_us is None:
                    if (
                        sync_mode_applied == "master"
                        and stop_trigger_event is not None
                        and stop_trigger_event.is_set()
                        and capture_end_ready_event is not None
                        and not capture_end_ready_event.is_set()
                    ):
                        common_end = compute_common_interval_end(
                            int(iterator.get_current_time()), capture_start_ts_us, int(delta_t_us)
                        )
                        publish_shared_int(shared_capture_end_ts_us, common_end)
                        capture_end_ready_event.set()
                    if capture_end_ready_event is not None and capture_end_ready_event.is_set():
                        published_end = read_shared_int(shared_capture_end_ts_us)
                        if published_end < 0:
                            raise RuntimeError(f"{side}: invalid shared capture end timestamp {published_end}")
                        capture_end_ts_us = int(published_end)
                        meta["capture_end_ts_us"] = int(capture_end_ts_us)

                window_end_ts_us = (
                    capture_end_ts_us if capture_end_ts_us is not None else OPEN_INTERVAL_END_TS_US
                )
                selected = filter_events_to_window(events, capture_start_ts_us, window_end_ts_us)
                    ...
                    if sync_mode_applied == "master" and str(capture_start_marker).strip():
                        atomic_write_json(
                            Path(capture_start_marker).resolve(),
                            {
                                ...
                                "capture_end_ts_us": (
                                    int(capture_end_ts_us) if capture_end_ts_us is not None else -1
                                ),
                ...
                capture_complete = (
                    capture_end_ts_us is not None
                    and int(iterator.get_current_time()) >= capture_end_ts_us
                )
            else:
        ...
        if hardware_synchronized:
            if capture_end_ts_us is None:
                raise RuntimeError(f"{side}: the saved interval never received a shared end timestamp")
            output_start_ts_us = int(capture_start_ts_us)
            output_end_ts_us = int(capture_end_ts_us)
            sync_ts_us = int(capture_start_ts_us)
            meta["capture_duration_sec"] = float(output_end_ts_us - output_start_ts_us) / 1e6
        else:
            output_start_ts_us = int(capture_start_ts_us)
...
def main() -> int:
    args = parse_args()
    operator_triggered = str(args.start_trigger) == "enter"
    manual_stop = float(args.duration_sec) <= 0
    if float(args.duration_sec) < 0:
        raise SystemExit("--duration-sec must be zero or positive.")
    if manual_stop and not operator_triggered:
        raise SystemExit(
            "--duration-sec must be positive unless --start-trigger enter is used, "
            "where 0 means a second Enter ends the interval."
        )
    if manual_stop and args.hw_sync == "off":
        raise SystemExit(
            "--duration-sec 0 needs hardware synchronization so both cameras can "
            "share one end timestamp. Use --hw-sync left-master or right-master."
        )
    ...
    start_trigger_event = ctx.Event() if operator_triggered else None
    stop_trigger_event = ctx.Event() if manual_stop else None
    capture_end_ready_event = ctx.Event() if manual_stop else None
    shared_capture_end_ts_us = ctx.Value("q", -1) if manual_stop else None
    trigger_kwargs = {
        "start_trigger_event": start_trigger_event,
        "stop_trigger_event": stop_trigger_event,
        "capture_end_ready_event": capture_end_ready_event,
        "shared_capture_end_ts_us": shared_capture_end_ts_us,
    }
    ...
    if operator_triggered:
        print(
            "Saved interval starts on your Enter, in the Master camera clock"
            + (" and ends on a second Enter." if manual_stop else f" and runs {args.duration_sec:.3f} s.")
        )
    elif args.hw_sync == "off":
    ...
    recorded_sec = 0.0
    if operator_triggered and started_worker_indices and not abort_event.is_set():
        if not prompt_operator(
            "[REC] Both cameras are streaming. Enter starts the saved interval (Q aborts): "
        ):
            abort_event.set()
            print("[REC] Aborted before the saved interval started.")
        else:
            start_trigger_event.set()
            trigger_monotonic = time.monotonic()
            print("[REC] Armed. The Master is publishing the shared start timestamp.")
            if manual_stop:
                if not prompt_operator("[REC] Recording. Enter stops it (Q aborts): "):
                    abort_event.set()
                    print("[REC] Aborted while recording.")
                else:
                    stop_trigger_event.set()
                    recorded_sec = time.monotonic() - trigger_monotonic
                    print(f"[REC] Stop published after about {recorded_sec:.3f} s.")

    results: list[dict[str, Any]] = []
    # Measured from here, so the operator may take as long as they like deciding.
    deadline = time.monotonic() + max(float(args.duration_sec), recorded_sec) + float(args.start_delay_sec) + 60.0
