...
def configure_local_metavision_environment() -> None:
    ...
    openeb_install_dir = Path(os.environ.get("EVENTCAM_OPENEB_ROOT", str(project_dir / "openeb_install")))
    ...
    if os.environ.get("EVENTCAM_VCPKG_BIN"):
        vcpkg_bin_dir = Path(os.environ["EVENTCAM_VCPKG_BIN"])
    ...
    centuryarks_bin_dir = Path(os.environ.get("EVENTCAM_CAMERA_BIN_DIR", str(centuryarks_dir / "bin")))
    centuryarks_plugin_dir = Path(os.environ.get("EVENTCAM_CAMERA_PLUGIN_DIR", str(centuryarks_dir / "plugins")))
