...
# -*- coding: utf-8 -*-
"""
acoustools_eventcam_multitraj_kd_no-auto_trigger_in.py
======================================================
...
camera recorder. Camera acquisition runs in a background thread so that the
main thread can send the trajectory to OpenMPD/AcousTools with minimal Python
side work during motion. After each run the recorded RAW event stream is
converted to a review video by ``eventcam_raw_postprocess.py``.
...
# User settings
# =========================
SAVE_DIR = "./recordings_eventcam_acoustools"  # イベントカメラRAW/同期ログ/軌道ログの保存先
...
# EVENTCAM_POSTPROCESS_VIDEO_FPS = 50.0  # MP4に書き込む再生fps。50ならスロー再生になる
# EVENTCAM_POSTPROCESS_VIDEO_FPS = 2000.0  # MP4に書き込む再生fps。50ならスロー再生になる
# EVENTCAM_POSTPROCESS_ACCUMULATION_US = 200  # 1枚の画像に積算する時間幅。短いほど軌跡の残像が減る
EVENTCAM_POSTPROCESS_FPS = 5000.0
EVENTCAM_POSTPROCESS_VIDEO_FPS = 50.0
EVENTCAM_POSTPROCESS_ACCUMULATION_US = int(round(1_000_000 / EVENTCAM_POSTPROCESS_FPS))
EVENTCAM_POSTPROCESS_OUTPUT_DIR = "./processed_eventcam_acoustools"
EVENTCAM_POSTPROCESS_IN_BACKGROUND = True  # Trueなら動画変換を別スレッドで開始する
...
class EventCameraRecorder:
    ...
    def __init__(
        ...
        *,
        ...
        postprocess_in_background: bool = EVENTCAM_POSTPROCESS_IN_BACKGROUND,
        ...
        self.postprocess_in_background = bool(postprocess_in_background)
    ...
    def _configure_and_import_metavision(self) -> None:
        from eventcam_enter_capture import configure_local_metavision_environment
    ...
    def _start_postprocess(self, summary: EventCameraRecordingSummary) -> None:
        ...
        script_path = Path(__file__).resolve().parent / "eventcam_raw_postprocess.py"
        if not script_path.exists():
            print(f"[EVENTCAM][WARN] postprocess script not found: {script_path}")
            return
        ...
        cmd = [
            ...
            "--accumulation-us",
            str(max(0, int(self.postprocess_accumulation_us))),
            "--output-dir",
            str(self.postprocess_output_dir),
        ]
        summary.postprocess_command = cmd

        def _run() -> None:
            print(
                f"[EVENTCAM] Starting conversion: render={self.postprocess_fps:g} fps, "
            ...
            try:
                ...
                print("[EVENTCAM] Postprocess finished.")
            except Exception as exc:
                print(f"[EVENTCAM][WARN] postprocess failed: {exc}")

        if self.postprocess_in_background:
            thread = threading.Thread(target=_run, name="EventCameraPostprocess", daemon=False)
            thread.start()
            self.postprocess_threads.append(thread)
        else:
            _run()
    ...
    def close(self) -> None:
        ...
        for thread in list(self.postprocess_threads):
            try:
                thread.join()
            except Exception:
                pass
...
def main() -> None:
    ...
    # Instantiate the event camera recorder
    ...
    try:
        recorder = EventCameraRecorder(
            ...
            postprocess_in_background=EVENTCAM_POSTPROCESS_IN_BACKGROUND,
    ...
    try:
        while True:
            ...
            # 5. Open a lightweight preview before recording, then close it before RAW logging starts.
            ...
            # 10b. Event-camera RAW is converted to a 1000 fps MP4 by EventCameraRecorder.stop_recording().
