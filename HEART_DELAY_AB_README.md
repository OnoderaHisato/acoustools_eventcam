...
- `compare_heart_delay_ab.py` (used after stereo postprocessing)
...
The dry run prints the generated x/y/z span. Do not continue if all spans are
zero; a 7 mm heart must produce a non-degenerate trajectory.
...
## Automatic capture, postprocess, and comparison

Pass `--postprocess-and-compare` to postprocess all successful captures and
then aggregate the A/B result. PAT and camera recording are shut down before
CPU-heavy postprocessing starts.

```powershell
.\venv\Scripts\python.exe .\acoustools_stereo_heart_delay_ab.py `
  --output-dir .\stereo_acoustools_heart_delay_ab_records\full `
  --heart-scale-mm 7 `
  --heart-frequency-hz 10 `
  --steps-per-cycle 800 `
  --loops 10 `
...
For Heart A/B only, camera capture defaults to the nominal PAT duration plus
0.3 s. The standard 10 Hz, 10-loop experiment therefore records 1.3 s rather
than 3.5 s. The generic recording core keeps its existing margins. Override
the Heart defaults explicitly with `--post-roll-sec` and
`--capture-tail-margin-sec` when extra margin is required.
