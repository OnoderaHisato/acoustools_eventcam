...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Process a stereo_eventcam_record.py run: track both NPZs, then triangulate 3D.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("run_dir", type=Path, help="Run folder created by stereo_eventcam_record.py.")
    parser.add_argument(
        "--stereo-calibration",
        type=Path,
        default=Path(
