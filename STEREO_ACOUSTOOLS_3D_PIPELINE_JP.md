...
### LED検出の整合チェック（2026-09-18追加、既定は警告のみ）

LEDが見えていないときに、広い探索窓の中のノイズ1ビンをPAT開始として採用してしまう誤検出が
起きています（2026-09-16に1本、2026-09-18に2本）。検出そのものの条件は従来と同じまま、
検出結果が次のどちらかに当たる場合に**警告を出して記録に残します。runは従来どおり保存されます。**

- onsetが「PC時刻の見込み＋PAT送信の開始遅延」から0.1 sより離れている。
  過去248本の記録では、正しい検出は見込みの +4〜+14 ms に集中し、最大のずれは −55 ms でした。
  誤検出は −1.1 s と −1.4 s でした。
- peakが閾値の1.5倍未満である。peakが閾値と同じ検出はノイズと区別できません。

警告は画面に `[SYNC][LED][WARN]` として出て、`pat_camera_timing.json` の
`pat_start_led_consistency` と、`pipeline_manifest.json` の `pat_start_led.consistency`
（`suspicious`、`warnings`）に残ります。後処理やスパコン転送の前に `suspicious=true` のrunを確認してください。
数値は `*_pat_start_led.json` の `onset_residual_sec` と `peak_to_threshold_ratio` にあります。

当初は既定でこの2条件を満たさない候補を拒否していましたが、実機でLEDが点灯しているのに
検出失敗が続いたため、既定を警告のみへ戻しました（2026-09-18夜）。拒否したい場合だけ
`--pat-start-led-onset-tolerance-sec 0.1` と `--pat-start-led-min-peak-ratio 1.5` を指定します。
探索窓、閾値の決め方、onsetの定義、ROIは変えていません。

LED検出や記録に失敗した理由は、失敗runが削除された後も、auto sessionのJSONの
各runの `capture_report.failures` に残ります。

### 記録失敗時の撮り直しとカメラ停止の監視（2026-09-18追加）

- 記録プロセスは、保存区間の途中でどちらかのカメラからデータが3秒届かなくなると、
  「どちらのカメラのイベント配信が止まったか」を表示してすぐに失敗します
  （`--camera-stall-timeout-sec`、0で無効）。従来は約45秒待ってから打ち切られ、
  原因の表示が出ないことがありました。
- 記録プロセスの失敗・タイムアウトや、同期記録が始まらなかった場合も、LEDの失敗と同じく
  「同じ軌道を再計測しますか？ (Y/n)」と尋ねます。`Y` または Enter で、計算済みのホログラムの
  まま撮り直します。`n` のときだけ、従来どおりそのrunを削除して終了します。
  無人モード（`--unattended-after-first-checkpoint`）では、従来どおり上限回数まで自動で撮り直します。
...
- LED検出の整合チェック: 警告のみ（見込みから0.1 s超のずれ、またはpeakが閾値の1.5倍未満で警告。拒否はしない）
- カメラのイベント配信停止の判定: 保存区間中に3 sデータなし
- 2D補間する最大欠落時間: 5 ms
